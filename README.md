...
The binary is static, with no dependencies. 
...
#### Arch Linux
mosdepth is available as an [AUR package](https://aur.archlinux.org/packages/mosdepth/).

### Install with Docker
A Docker image is provided on quay:

```docker
docker pull quay.io/biocontainers/mosdepth:0.3.11--h0ec343a_1
docker run -v /hostpath/:/opt/mount quay.io/biocontainers/mosdepth:0.3.11--h0ec343a_1 mosdepth --by 1000 /opt/mount/$sample /opt/mount/$bam
```


### Building from Source

`mosdepth` requires `htslib` 1.4 or later.
 If you get an error
...
See [install.sh](https://github.com/brentp/mosdepth/blob/master/scripts/install.sh) for more details.
...
given a set of regions to the `--by` argment, `mosdepth` can report the number of bases in each region that are covered at or above each threshold value given to `--thresholds`. For example, 
```shell
...
If there is no name (4th) column in the bed file sent to `--by`, then that column will contain "unknown"
in the output.
...
This creates bins of [0:1), [1:4), [4:100), [100:200), [200:infinity), where the upper endpoint is non-inclusive.
...
D4 is a format created by [Hao Hou](https://github.com/38) in the Quinlan lab that drastically improves write speed.

For one test-case, it took **24.8s** to write a per-base.bed.gz compared to **7.7s** to write a d4 file. 

For the same case, running `mosdepth` without writing per-base takes 5.9 seconds so D4 greatly mitigates the cost of outputting per-base depth **and** makes the output more useful.

As of version 0.3.0, it has been incorporated into `mosdepth` for per-base output with the `--d4` flag.

## Examples

### Exome Coverage Example
...
`-n` skips outputting per-base data. This makes `mosdepth`
...
and also allows htslib to extract less data from the CRAM, providing a substantial speed improvement.
...
Here, regions with a depth of 0 are labelled as "NO_COVERAGE", those with a depth of 1,2,3,4 are labelled as "LOW_COVERAGE", and so on.

The result is a BED file where adjacent bases with depths in the same bin are merged into a single region. The 4th column indicates the label.


### Distribution with modified precision only
...
The `$prefix.mosdepth.global.dist.txt` file contains a cumulative distribution indicating the proportion of total bases (or the proportion of the `--by` for `$prefix.mosdepth.region.dist.txt`) that were covered
...
A python plotting script is provided in `scripts/plot-dist.py` that will make plots like below. 
```shell
python scripts/plot-dist.py \*global.dist.txt
```
The output is a `dist.html` with a plot for the full set along with one for each chromosome.

Using this, we can plot the distribution from the entire genome.
...
We can also view the Y chromosome to verify that males and females track separately. Below, we that see female samples cluster along the axes while male samples have
...
For every start it encounters, it increments the value in that position of the array. For every stop, it decrements that position. From this, the depth at a particular position is the cumulative sum of all array positions preceding it (a similar algorithm is used in BEDTools where starts and stops are tracked separately). 

`mosdepth` **avoids double-counting overlapping mate-pairs** and  **tracks every aligned part of every read using the CIGAR operations**. Because of this data structure, the the coverage `distribution` calculation
...
The `mosdepth` method has some limitations: 
Because a large array is allocated and it is required (in general) to take the cumulative sum of all preceding positions to know the depth at any position, it is slower for small, 1-time regional queries. 
It is, however fast for window-based or BED-based regions, because it first calculates the full chromosome coverage and then reports the coverage for each region in that chromosome. 

Another downside is it uses more memory than samtools. The amount of memory is approximately equal to 32-bits * longest chrom length, so for the 249MB chromosome 1, it will require 1GB of memory.

`mosdepth` is written in [nim](https://nim-lang.org/) and uses [htslib](https://github.com/samtools/htslib) via our nim wrapper [hts-nim](https://github.com/brentp/hts-nim/).
